...
export type SendDrawCompletedWhatsAppOptions = {
	to: string;
	groupTitle: string;
	groupUrl: string;
};
...
const buildDrawCompletedWhatsAppConfig = (env?: EnvSource): WhatsAppCloudConfig => ({
	accessToken: readEnvValue(env, ['WHATSAPP_ACCESS_TOKEN', 'META_WHATSAPP_ACCESS_TOKEN']),
	phoneNumberId: readEnvValue(env, ['WHATSAPP_PHONE_NUMBER_ID', 'META_WHATSAPP_PHONE_NUMBER_ID']),
	apiVersion: readEnvValue(env, ['WHATSAPP_API_VERSION']) ?? 'v25.0',
	...
	templateComponents: parseComponentsMode(readEnvValue(env, ['WHATSAPP_DRAW_TEMPLATE_COMPONENTS'])),
	useNamedParameters: readEnvValue(env, ['WHATSAPP_DRAW_USE_NAMED_PARAMETERS']) === 'true',
...
const buildDrawCompletedTemplateComponents = (
	options: SendDrawCompletedWhatsAppOptions,
	config: WhatsAppCloudConfig
): TemplateComponent[] | undefined => {
	if (config.templateComponents === 'none') {
		return undefined;
...
export const sendDrawCompletedWhatsApp = async (
	...
	const config = buildDrawCompletedWhatsAppConfig(env);

	if (!isWhatsAppCloudConfigured(config)) {
		console.warn('[whatsapp-cloud] Credenciais ou template de sorteio não configurados.');
		return false;
	}

	const recipient = normalizeWhatsAppPhone(options.to, config.defaultCountryCode);
	if (!recipient) {
		console.warn('[whatsapp-cloud] Telefone inválido para sorteio concluído:', options.to);
		return false;
	}

	const components = buildDrawCompletedTemplateComponents(options, config);
	const template: Record<string, unknown> = {
		name: config.templateName,
		language: { code: config.templateLanguage }
	};
	...
	try {
		console.info(
			'[whatsapp-cloud] Payload de sorteio concluído:',
			JSON.stringify(
				{
					url,
					apiVersion: config.apiVersion,
		...
		if (!response.ok) {
			...
			const errorMessage = parsedResponse?.error?.message ?? responseBody;
			console.error(
				'[whatsapp-cloud] Falha ao enviar template de sorteio concluído:',
				JSON.stringify(
					{
						status: response.status,
						errorCode,
						errorType,
						errorMessage,
						hint: 'Confira WHATSAPP_DRAW_TEMPLATE_* no Meta e no ambiente.',
						raw: responseBody
					},
					null,
					2
		...
		console.info(
			'[whatsapp-cloud] Template de sorteio concluído aceito pela Meta:',
			JSON.stringify(
				{
					status: response.status,
					wamid,
		...
		console.error('[whatsapp-cloud] Erro inesperado ao enviar sorteio concluído:', error);
		return false;
	}
};
